...
schematic.enumerated().forEach { rowIndex, row in
	row.enumerated().forEach { columnIndex, character in
		guard character != ".", character.isSchematicChar else { return }
		partNumbers.insert(contentsOf: findPartNumbers(around: .init(row: rowIndex, column: row.index(row.startIndex, offsetBy: columnIndex))), at: 0)
	}
}
