...
extension Character {
	var isSchematicChar: Bool {
		return !isNumber && !isLetter && self != "."
	}
}
struct SymbolLocation {
	...
	public func translate(in direction: DiagonalDirection) -> SymbolLocation {
		switch direction {
			case .topLeft:
				return .init(row: self.row - 1, column: schematic[row].index(self.column, offsetBy: -1))
			case .topRight:
				return .init(row: self.row - 1, column: schematic[row].index(self.column, offsetBy: 1))
			case .bottomLeft:
				return .init(row: self.row + 1, column: schematic[row].index(self.column, offsetBy: -1))
...
var identifiedLocations = [(row: Int, range: ClosedRange<String.Index>)]()

extension Array where Element == String {
	func symbol(at location: SymbolLocation) -> String.Element {
		return self[location.row][location.column]
	}
	
	func isValid(location: SymbolLocation) -> Bool {
		return location.row >= 0 && location.row < count
			  && location.column >= schematic[location.row].startIndex
			  && location.column < schematic[location.row].endIndex
	}
}

func findWholePartNumber(around location: SymbolLocation) -> Int? {
	let row = schematic[location.row]
	
	var firstIndex = location.column
	var lastIndex = location.column
	
	var currentIndex = location.column
	// look left
	while currentIndex >= row.startIndex, row[currentIndex].isNumber {
		firstIndex = currentIndex
		if currentIndex == row.startIndex {
			break
		}
		currentIndex = row.index(currentIndex, offsetBy: -1)
	...
	//look right
	while currentIndex < row.endIndex, row[currentIndex].isNumber {
		lastIndex = currentIndex
		currentIndex = row.index(currentIndex, offsetBy: 1)
	}
	guard !identifiedLocations.contains(where: { 
		$0.row == location.row && $0.range == firstIndex...lastIndex
	}),
		let partNumber = Int(row[firstIndex...lastIndex])
		else { return nil }
		identifiedLocations.append((row: location.row, range: firstIndex...lastIndex))
	return partNumber
	
	
}
func findPartNumber(leftOf location: SymbolLocation) -> Int? {
	let row = schematic[location.row]
	let searchIndex = row.index(location.column, offsetBy: -1)
	
	guard searchIndex >= row.startIndex, row[searchIndex].isNumber else { return nil }
	return findWholePartNumber(around: .init(row: location.row, column: searchIndex))
	
}

func findPartNumber(rightOf location: SymbolLocation) -> Int? {
	let row = schematic[location.row]
	let searchIndex = row.index(location.column, offsetBy: 1)
	
	guard searchIndex < row.endIndex, row[searchIndex].isNumber else { return nil }
	return findWholePartNumber(around: .init(row: location.row, column: searchIndex))
}

func findPartNumber(above location: SymbolLocation) -> Int? {
	
	let searchLocation = SymbolLocation(row: location.row - 1, column: location.column)
	
	guard searchLocation.row >= 0,
		schematic.symbol(at: searchLocation).isNumber
	else { return nil }
	return findWholePartNumber(around: searchLocation)
	
	
}

func findPartNumber(below location: SymbolLocation) -> Int? {
	let searchLocation = SymbolLocation(row: location.row + 1, column: location.column)
	
	guard searchLocation.row < schematic.count,
		  schematic.symbol(at: searchLocation).isNumber 
	else { return nil }
	return findWholePartNumber(around: searchLocation)
}

func findPartNumber(diagonallyFrom location: SymbolLocation, in direction: DiagonalDirection) -> Int? {
	let searchLocation = location.translate(in: direction) 
	guard schematic.isValid(location: searchLocation),
		  schematic.symbol(at: searchLocation).isNumber
			else { return nil }
	return findWholePartNumber(around: searchLocation)
}

func findPartNumbers(diagonallyFrom location: SymbolLocation) -> [Int?] {
	return [
		findPartNumber(diagonallyFrom: location, in: .topLeft),
		findPartNumber(diagonallyFrom: location, in: .topRight),
		findPartNumber(diagonallyFrom: location, in: .bottomLeft),
		findPartNumber(diagonallyFrom: location, in: .bottomRight)
	]
}

func findPartNumbers(around location: SymbolLocation) -> [Int] {
	print("Character encountered: \(schematic.symbol(at: location)), at row: \(location.row), column: \(schematic[location.row].distance(from: schematic[location.row].startIndex, to: location.column))")
	var partNumbers = [Int?]()
	partNumbers.append(findPartNumber(above: location))
	partNumbers.append(findPartNumber(below: location))
	partNumbers.append(findPartNumber(leftOf: location))
	partNumbers.append(findPartNumber(rightOf: location))
	partNumbers.insert(contentsOf: findPartNumbers(diagonallyFrom: location), at: partNumbers.endIndex)
	print("Found: \(partNumbers.compactMap{$0})")
	return partNumbers.compactMap{$0}
}

func findPartNumbersAdjacentTo(symbol: Character) -> [[Int]] {
	var partNumbers = [[Int]]()
	schematic.enumerated().forEach { rowIndex, row in
		row.enumerated().forEach { columnIndex, character in
			guard character == "*" else { return }
			partNumbers.append(findPartNumbers(around: .init(row: rowIndex, column: row.index(row.startIndex, offsetBy: columnIndex))))
...
let gearsPartNumbers = findPartNumbersAdjacentTo(symbol: "*")

let sum = gearsPartNumbers.reduce(0) {
	guard $1.count > 1 else { return $0 }
	return $0 + $1.reduce(1) {
		$0 * $1
	}
}
