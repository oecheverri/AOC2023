...
typealias NumberLocation = (index: Int, value: String)
typealias SpelledNumber = (String, String)
let spelledNumbers = ["one" : "1", "two" : "2", "three": "3", "four" : "4", "five" : "5", "six" : "6", "seven": "7", "eight": "8", "nine": "9"]


func findNumberLocations(string: String) -> [NumberLocation] {
	
	var numberLocations: [NumberLocation] = string.enumerated().reduce([]) {
		if $1.element.isNumber {
			var res = $0
			res.append(NumberLocation(index: $1.offset, value: String($1.element)))
			return res
		}
		return $0
	}
	...
	spelledNumbers.forEach {
		numberLocations = findLocationsOf(spelledNumber: $0, in: string)
		.reduce(numberLocations) {
			var res = $0
			res.append($1)
			return res
		}
...
func findLocationsOf(spelledNumber: SpelledNumber, in string: String) -> [NumberLocation] {
	return string.ranges(of: spelledNumber.0)
	.map {
		(index: string.distance(from: string.startIndex, to: $0.lowerBound), spelledNumber.1)
	}
}

func getNumber(numbers: [NumberLocation]) -> Int? {
...
.compactMap {
	getNumber(numbers: findNumberLocations(string: $0))
}
.reduce(0) {
	$0 + $1
}
