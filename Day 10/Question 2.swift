...
enum Tile: String, CaseIterable {
    ...
    var travelDirections: [Direction] {
        switch self {
            case .NorthSouth:
            return [.North, .South]
            case .EastWest:
    ...
    func isParallel(to direction: Direction) -> Bool {
        switch self {
            case .NorthSouth:
    ...
    func isIntersected(in direction: Direction) -> Bool {
        
        if self == .Ground || self == .Start {
            return false
        }
        ...
        if isCorner {
            switch direction {
                case .North:
                    fallthrough
                case .South:
                    return travelDirections.contains(.East)
                case .East:
                    fallthrough
                case .West:
                    return travelDirections.contains(.North)
                case .None:
                    return false
                    
            }
        }
        return !isParallel(to: direction)
    }
    
    func canEnterFrom(direction: Direction) -> Bool {
        travelDirections.contains(direction)
    }
    
    func canTravelIn(direction: Direction) -> Bool {
        travelDirections.contains(!direction)
    }
}
...
class Row {
    ...
    init(tiles: [Tile]) {
        self.tiles = tiles
        distances = .init(repeating: 0, count: tiles.count)
    }
    
    func firstIndexOf(tile: Tile) -> Int? {
        return tiles.firstIndex(of: tile)
    }
}
...
struct Coordinate: Hashable, CustomStringConvertible {
    ...
    func translated(in direction: Direction) -> Coordinate {
        .init(x: self.x + direction.translation.x, y: self.y + direction.translation.y)
    }
...
.map{
    Row(tiles: $0.map(String.init).compactMap(Tile.init))
}

var startCoordinates: Coordinate = {
    for (index, row) in map.enumerated() {
        guard let column = row.firstIndexOf(tile: .Start) else { continue }
        return .init(x: column, y: index)
    }
    return .init(x: -1, y: -1)
}()

func findNextStep(from coords: Coordinate, lastdirection direction: inout Direction) -> Coordinate {
    let tile = map[coords.y].tiles[coords.x]
    direction = tile.travelDirections.filter{ $0 != !direction }[0]
    
    let nextX = coords.x + direction.translation.x
    let nextY = coords.y + direction.translation.y
    
    return .init(x: nextX, y: nextY)
}

func findStartTileType(coords: Coordinate) -> Tile {
    var possibleDirections = [Direction]()
    let scanDirections = [Direction.North, .South, .East, .West]
    
    for scanDirection in scanDirections {
        let translation = scanDirection.translation
        let newCoords = (x: coords.x + translation.x, y: coords.y + translation.y)
        
        guard newCoords.y >= map.startIndex,
        newCoords.y < map.endIndex,
        newCoords.x >= map[newCoords.y].tiles.startIndex,
        newCoords.x < map[newCoords.y].tiles.endIndex
        else { continue }
        
        let tile = map[newCoords.y].tiles[newCoords.x]
        if tile.canTravelIn(direction: scanDirection) {
            possibleDirections.append(scanDirection)
        }
    }
    
    for tile in Tile.allCases {
        if possibleDirections.sorted() == tile.travelDirections.sorted() {
            return tile
        }
    }
    return .Ground
...
map[startCoordinates.y].tiles[startCoordinates.x] = findStartTileType(coords: startCoordinates)

var lastDirection = Direction.None
var loopCoordinates = Set<Coordinate>()

repeat {
    currentCoordinates = findNextStep(from: currentCoordinates, lastdirection: &lastDirection)
    loopCoordinates.insert(currentCoordinates)
    steps += 1
} while currentCoordinates != startCoordinates

func isInBounds(coordinate: Coordinate) -> Bool {
    return coordinate.y >= map.startIndex
    && coordinate.y < map.endIndex
    && coordinate.x >= map[coordinate.y].tiles.startIndex
    && coordinate.x < map[coordinate.y].tiles.endIndex
    
}

func castRayCountingIntersections(with coordSet: Set<Coordinate>, in direction: Direction, from coord: Coordinate) -> Int {
    var intersectionCount = 0
    var currentRayCoordinate = coord
    while isInBounds(coordinate: currentRayCoordinate) {
        currentRayCoordinate = currentRayCoordinate.translated(in: direction)
        if coordSet.contains(currentRayCoordinate) {
            let tile = map[currentRayCoordinate.y].tiles[currentRayCoordinate.x]
            if tile.isIntersected(in: direction) {
                intersectionCount += 1
            }
        }
    }
...
func isInsideLoop(coord: Coordinate, loopCoords: Set<Coordinate>) -> Bool {
    let testDirections = [Direction.North, .South, .East, .West]
    guard !loopCoords.contains(coord) else { return false }
    
    for direction in testDirections {
        let intersections = castRayCountingIntersections(with: loopCoords, in: direction, from: coord)
...
for (y, row) in map.enumerated() {
    for x in row.tiles.indices {
        inLoopCount += isInsideLoop(coord: .init(x: x, y: y), loopCoords: loopCoordinates) ? 1 : 0
    }
}

print("Tiles in loop: \(inLoopCount)")
