...
struct Draw {
	...
	init?(from string: String) {
		...
		guard components.count == 2,
			  let colour: Colour = .init(rawValue: components[1]),
...
struct Round {
	var redDraw: Draw? = nil
	var greenDraw: Draw? = nil
	var blueDraw: Draw? = nil
	
	init?(from string: String) {
		let drawStrings = string.components(separatedBy: ", ")
		for drawString in drawStrings {
			guard let draw: Draw = .init(from: drawString) else { continue }
			switch draw.colour {
				case .red:
					if redDraw != nil { return nil }
					redDraw = draw
				case .green:
					if greenDraw != nil { return nil }
					greenDraw = draw
				case .blue:
					if blueDraw != nil { return nil }
					blueDraw = draw
			}
		}
	}
...
struct Game {
	...
	init?(from string: String) {
		let components = string.components(separatedBy: ": ")
		guard components.count == 2,
			  let id = Int(components[0].components(separatedBy: " ")[1])
		else { return nil }
		self.id = id
		let roundStrings = components[1].components(separatedBy: "; ")
		var rounds = [Round]()
		for roundString in roundStrings {
			guard let round = Round(from: roundString) else { continue }
			rounds.append(round)
		}
		self.rounds = rounds
	}
