...
let sum = input.components(separatedBy: "\n").compactMap {
	let result = $0.reduce((first: "", last: "")){
		guard $1.isNumber
		else { return $0 }
		if $0.first.isEmpty {
			return (first: String($1), last: "")
		} else {
			return (first: $0.first, last: String($1))
		}
	}
	return Int(result.first + (result.last.isEmpty ? result.first : result.last))
}.reduce(0) { 
	$0 + $1
}
